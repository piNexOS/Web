...
namespace Web.Pages.Gerenciamento.Os
{
    public class OsModel : PageModel
    {
        ...
        public IActionResult OnGet()
        {
            ...
            List<string> ListStatusRaw = new List<string> { "PROGRAMADA", "NÃO PROGRAMADA" };
            ListStatus = new SelectList(ListStatusRaw);
        ...
        public PartialViewResult OnGetPartialView(int idMunicipio, int idBairro, int idServico,String status, DateTime dataVencimento)
        {
            ordemServicoRepository = new Infra.Repositories.OrdemServicoRepository();
            PartialViewModel = new PartialViewModelType();
            PartialViewModel.OrdensServico = ordemServicoRepository.GetOrdensServicosParaGerenciar(idMunicipio, idBairro,idServico,status, dataVencimento);
